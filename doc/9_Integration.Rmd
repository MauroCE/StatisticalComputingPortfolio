...
### Plotting Function
The following function can be called to run one of the sampling algorithm with a given proposal and a given target density. It will plot a density of the target distribution and a histogram of the samples generated by the algorithm.
...
# named lists
gamma <- list("density"=gamma_density, "sample"=gamma_sample, "name"="Gamma", "range"=c(0.0, 20.0))
beta <- list("density"=beta_density, "sample"=beta_sample, "name"="Beta", "range"=c(0.0, 1.0))
normal <- list("density"=normal_density, "sample"=normal_sample, "name"="Normal")
chisq <- list("density"=chisq_density, "sample"=NULL, "name"="Chi-Squared", "range"=c(0.0, 20.0))
exponential <- list("density"=dexp, "sample"=exponential_sample, "name"="Exponential", "range"=c(0.0, 6.0))
```
...
&= \sum_{i=1}^n y_i \ln(p_i) + (1 - y_i)\ln(1 - p_i) + -\frac{1}{2\sigma_{\vbeta}^2}\vbeta^\top \vbeta \\
&\propto -\sigma^2_{\vbeta}\sum_{i=1}^n\ln\left(1 + \exp((1 - 2y_i)\vx_i^\top\vbeta)\right) - \frac{1}{2}\vbeta^\top\vbeta
\end{align}
$$
...
$$
x_2 = -\frac{\beta_2}{\beta_3}x_1 -\frac{\beta_1}{\beta_3}
$$
...
dfmap <- data.frame(x1=X[, 2], y=(-start[2, ]/start[3, ])*X[, 2] + (-start[1, ]/start[3, ]))
ggplot() +
