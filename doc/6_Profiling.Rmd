...
Add some theory of Gaussian Processes
...
profvis(result <- gp_naive(xtrain, ytrain, xtest, sigma_n))
```
...
profvis(result_online <- gp_online(xtrain, ytrain, xtest, sigma_n))
```
...
profvis(gp_online_vect(xtrain, ytrain, xtest, sigma_n))
```
...
profvis(gp_completely_vectorized(xtrain, ytrain, xtest, sigma_n), interval=0.005)
```
...
```{r}
Kss <- kernel_matrix_vectorized(xtest, compute_sigmasq(xtest))
...
```{r}
# Get predicitons
#results <- gp_completely_vectorized(xtrain, ytrain, xtest, sigma_n)
results <- gp_naive(xtrain, ytrain, xtest, sigma_n)
gpmean <- results[[1]]
gpvcov <- results[[2]]
# Plot
data.frame(x=xtest, t(mvrnorm(npost_samples, gpmean, gpvcov))) %>% 
  ...
  ggplot(aes(x=x, y=Value)) + 
    ...
    ggtitle(paste(npost_samples, "MVN Samples after seeing the data")) + 
    theme(plot.title=element_text(hjust=0.5))
